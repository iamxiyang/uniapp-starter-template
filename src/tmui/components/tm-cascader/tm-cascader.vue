<template>
  <tm-sheet :color="props.color" :height="props.height" :padding="[0, 0]">
    <tm-sheet :color="props.color" text :round="3" :border="0" _class="flex flex-row overflow" :height="68" :margin="[0, 0]" :padding="[24, 0]">
      <view style="height: 68rpx" class="flex flex-row flex-row-center-start">
        <view v-for="(item, index) in _idArrays_str" :key="index" class="flex flex-row flex-row-center-start">
          <view>
            <tm-text :followTheme="_activeIndex == index ? props.followTheme : false" :fontSize="28" _class="text-overflow-1" :color="_activeIndex == index ? props.activeColor : ''" :label="item" @click="tabClick(index)"></tm-text>
          </view>
        ...
        <tm-text :followTheme="_activeIndex == _nowLevel ? props.followTheme : false" :color="_activeIndex == _nowLevel ? props.activeColor : ''" v-if="_nowLevel == _idArrays.length" :fontSize="28" _class="ml-10" label=">请选择" @click="tabClick(_nowLevel)"></tm-text>
      </view>
    </tm-sheet>
    ...
    <view style="height: 16rpx"></view>
    <BaseCascader :followTheme="props.followTheme" :color="props.activeColor" :height="props.height - 120 - slotTabHeigth" :data="listData" :beforeCellClick="props.beforeCellClick" @cell-click="emits('cell-click', $event)"></BaseCascader>
  </tm-sheet>
</template>
<script lang="ts" setup>
  ...
  import type { childrenData } from './interface'
