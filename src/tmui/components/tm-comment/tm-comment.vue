  ...
  >
    <view class="flex flex-row flex-row-top-start">
      <view v-if="ctl.avatar" class="mr-24" style="min-width: 60rpx; align-items: stretch">
        <tm-avatar :round="12" @click="emits('avatar-click', $event)" :followTheme="props.followTheme" :dark="props.dark" :followDark="props.followDark" :size="60" :img="ctl.avatar"></tm-avatar>
      </view>
      <view class="flex flex-9 flex-col wh5">
        <view class="flex flex-row" :class="[_align.time]">
          <slot name="author">
            <tm-text @click="emits('author-click', $event)" :followTheme="props.followTheme" :dark="props.dark" :followDark="props.followDark" :color="ctl.authorColor" :fontSize="ctl.authorFontSize" :label="ctl.author"></tm-text>
          </slot>
          <slot name="time">
            <tm-text @click="emits('time-click', $event)" v-if="_time" :followTheme="props.followTheme" :dark="props.dark" :followDark="props.followDark" _class="pl-24 opacity-5" :fontSize="22" :label="_time"></tm-text>
          </slot>
        </view>
        <view class="flex flex-col py-12">
          <slot name="content">
            <tm-text @click="emits('content-click', $event)" :followTheme="props.followTheme" :dark="props.dark" :followDark="props.followDark" :fontSize="28" :label="ctl.content"> </tm-text>
          </slot>
        </view>
