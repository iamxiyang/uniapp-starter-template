      ...
      <view v-for="(item2, index2) in _data" :key="index2">
        <view class="flex flex-row flex-row-center-center" :style="[{ height: '120rpx', flexWrap: 'wrap' }]">
          <view class="flex-4" :style="{ height: '112rpx' }" v-for="(item, index) in item2" :key="index">
            <tm-sheet @click="clickWeek(item)" :height="112" :shadow="0" :round="4" _class="flex-row flex-center" :text="_nowMonth == item.month" :color="_nowMonth == item.month ? _color : 'grey-4'" :margin="[4, 4]" :padding="[0, 0]">
              <view :style="[{ opacity: item.isVaild ? '0.3' : '1' }]" :userInteractionEnabled="false" style="width: 62rpx" class="flex-center flex-col">
...
<script lang="ts" setup>
  ...
  import type { monthYearItem } from './interface'
