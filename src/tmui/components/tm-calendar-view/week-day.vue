  ...
  <view class="flex flex-col relative">
    ...
    <view class="flex flex-col">
      <view class="flex flex-row flex-row-center-center" :style="[{ height: '74rpx' }]" v-for="(item, index) in _dataWeek" :key="index">
        ...
        <tm-sheet no-level @click="clickWeek(item)" :height="66" :shadow="0" :round="10" _class="flex-row" class="flex-6" paren-class="flex-6" :text="_weekNum == item" :color="_weekNum == item ? _color : 'grey-4'" :margin="[0, 4]" :padding="[0, 0]">
          <view :userInteractionEnabled="false" style="width: 62rpx" :class="[item2.isNowIn ? '' : 'opacity-5']" class="flex-1 flex-center" v-for="(item2, index2) in _data[index]" :key="index2">
    ...
    <tm-button v-if="!props.hideButton" :linear="props.linear" :linear-deep="props.linearDeep" :color="props.color" @click="confirm" block :label="_confirmText" :margin="[0, 16]"></tm-button>
  </view>
</template>
...
<script lang="ts" setup>
  ...
  import type { weekItem } from './interface'
