<template>
  <view class="flex flex-col relative">
    ...
    <view class="flex flex-col">
      <view class="flex flex-row flex-row-center-center" :style="[{ height: '90rpx' }]" v-for="(item, index) in _data" :key="index">
        <view @click="clickWeek(item2)" class="flex-1 flex flex-row flex-row-center-center" v-for="(item2, index2) in item" :key="index2">
          <view
            :userInteractionEnabled="false"
            :class="['flex-1', startOrAnd(item2.dateStr) == 1 ? 'round-l-24' : '', startOrAnd(item2.dateStr) == 2 ? 'round-r-24' : '', startOrAnd(item2.dateStr) == 3 ? 'round-24' : '', !startOrAnd(item2.dateStr) ? 'round-24' : '']"
          ...
          >
            <tm-sheet
              ...
              :padding="[0, 0]"
            >
              <view :userInteractionEnabled="false" style="width: 90rpx; height: 90rpx" :class="[!item2.isNowIn ? 'opacity-6' : '']" class="flex-1 flex-center">
                <view style="width: 90rpx" class="flex-center flex-col" :style="[{ opacity: item2.disabled ? '0.3' : '1' }]">
                  <tm-text :font-size="28" :label="item2.date"></tm-text>
                  <tm-text v-if="(startOrAnd(item2.dateStr) == 0 || startOrAnd(item2.dateStr) == 4) && item2.extra.extra" _class="flex-center" :font-size="22" :label="item2.extra.extra"></tm-text>
                  <tm-text v-if="startOrAnd(item2.dateStr) == 1" _class="flex-center" :font-size="22" label="开始"></tm-text>
                  <tm-text v-if="startOrAnd(item2.dateStr) == 2" _class="flex-center" :font-size="22" label="结束"></tm-text>
                  <tm-text v-if="startOrAnd(item2.dateStr) == 3" _class="flex-center" :font-size="20" label="始/止"></tm-text>
                </view>
              </view>
            </tm-sheet>
          </view>
    ...
    <tm-button v-if="!props.hideButton" :linear="props.linear" :linear-deep="props.linearDeep" :color="props.color" @click="confirm" block :label="_confirmText" :margin="[32, 16]"></tm-button>
  </view>
</template>
...
<script lang="ts" setup>
  ...
  import type { monthDayItem, dateItemStyle } from './interface'
