...
export class tmCv {
  ...
  public animation(arg: { duration?: number; repeat?: number; yoyo?: boolean } = { duration: 500, repeat: 0, yoyo: false }, onUpdate: (progress: number) => void, onStart?: (progress: number) => void): Promise<{ [key: string]: any }> {
    ...
    return new Promise((res, rej) => {
      ...
      setTimeout(() => {
        ...
        dom?.getComponentRect(ganvas, function (res: any) {
          ...
          // @ts-ignore
          getCanvas(t.proxy, domId, t.opts.width, t.opts.height)
            .then((e) => {
              t.ctx = e.ctx
              initOpts(e.node)
              res(t)
      ...
      setTimeout(() => {
        uni
          ...
          .exec((resNodes) => {
            ...
            // @ts-ignore
            getCanvas(proxy, this.canvasId, this.opts.width, this.opts.height)
              .then((e) => {
                initOpts(e.node)
                this.ctx = e.ctx
                stopDrawer = false
