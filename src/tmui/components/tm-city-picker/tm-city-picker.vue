<template>
  <view @click="showCity = !props.disabled ? !showCity : false">
    ...
    <slot></slot>
    <!-- #endif -->
    <tm-drawer :disabbleScroll="true" :round="props.round" ref="drawer" :height="820" :closable="true" :overlayClick="aniover" @open="drawerOpen" @cancel="cancel" @ok="confirm" :show="showCity" @update:show="closeDrawer" title="请选择地区" ok-text="确认" :duration="props.duration">
      <tm-picker-view
        v-if="showCity"
        ref="picker"
        :height="590"
      ...
      ></tm-picker-view>
      <tm-button label="确认选择" block :margin="[32, 12]" :color="props.color" :linear="props.linear" :linear-deep="props.linearDeep" @click="confirm" :round="props.btnRound"></tm-button>
