<template>
  <view class="flex flex-col flex-col-center-center">
    <view class="relative overflow" :class="[`round-${props.round} mx-${props.margin[0]} my-${props.margin[1]}`]" :style="[{ width: `${props.width}rpx`, height: `${props.height}rpx` }]">
      <swiper
      ...
      >
        <swiper-item :class="`round-${props.round} overflow `" @click="emits('click', index)" v-for="(item, index) in _list" :key="index" :style="[{ width: `${props.width}rpx`, height: `${props.height}rpx` }]">
          <tm-image :model="props.imgModel" :round="props.round" :userInteractionEnabled="false" :showLoad="props.showLoad" v-if="item.type == listItemType.img" :src="item.url" :width="props.width" :height="props.height"></tm-image>
          <tm-image :round="props.round" :userInteractionEnabled="false" :showLoad="props.showLoad" v-if="item.type == listItemType.video && item.img && _currentActive != index" :src="item.img" :width="props.width" :height="props.height"></tm-image>
          <video :userInteractionEnabled="false" id="video" v-if="item.type == listItemType.video && _currentActive === index" :src="item.url" :style="[{ width: `${props.width}rpx`, height: `${props.height}rpx` }]" :autoplay="_currentActive === index" :class="`round-${props.round}`"></video>
      ...
      >
        <tm-sheet :margin="[10, 10]" :follow-theme="_currentActive == index ? props.followTheme : false" :padding="[0, 0]" :round="10" @click="dotClick(index)" :color="_currentActive == index ? props.color : 'white'" v-for="(item, index) in _list" :key="index" :width="18" :height="18"></tm-sheet>
      </view>
      <!-- rect -->
      <view
        class="absolute"
      ...
      >
        <view class="round-10" :class="[_dotPosition == 'left' || _dotPosition == 'right' ? 'px-5 py-24 ' : '', _dotPosition == 'bottom' || _dotPosition == 'top' ? 'px-24 py-5 mx-32' : '']" style="background-color: rgba(0, 0, 0, 0.4); font-size: 0px">
...
<script lang="ts" setup>
  ...
  import type { listItem, listItemType, listItemTypeStr } from './interface'
