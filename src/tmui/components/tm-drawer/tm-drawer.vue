<template>
  <view>
    ...
    <tm-overlay :inContent="props.inContent" ref="overlayAni" :duration="props.duration + 80" @open="OverLayOpen" @close="overclose" :zIndex="props.zIndex" :transprent="!props.mask" @click="overlayClickFun" :align="align_rp" :teleport="props.teleport" :overlayClick="false" v-model:show="_show">
      <tm-translate :reverse="reverse_rp" :width="anwidth" :height="anheight" ref="drawerANI" :auto-play="false" :name="aniname" :duration="props.duration + 80" @end="playEndEvent">
        <view
          @click.stop="$event.stopPropagation()"
          :style="[{ width: anwidth, height: anheight }, !props.transprent ? tmcomputed.borderCss : '', !props.transprent ? tmcomputed.backgroundColorCss : '', !props.transprent ? tmcomputed.shadowColor : '', customCSSStyle]"
        ...
        >
          <view v-if="!props.closeable && !props.hideHeader" class="flex flex-row flex-row-center-center flex-between px-24" style="height: 44px">
            ...
            <view class="flex-4 flex-shrink flex-row flex-row-center-end">
              ...
              <tm-icon :color="okColor" v-if="ok_loading || loading" :spin="ok_loading || loading" :dark="isDark" :_class="isDark !== true ? 'opacity-4' : ''" :fontSize="24" :name="ok_loading || loading ? 'tmicon-shuaxin' : 'tmicon-times-circle-fill'"></tm-icon>
            </view>
          </view>
          <view v-if="props.closeable && !props.hideHeader" class="flex flex-row flex-row-center-center flex-between px-24" style="height: 44px">
