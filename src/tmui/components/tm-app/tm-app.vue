<template>
  <!-- #ifdef APP-NVUE -->
  <view class="flex relative" :style="[appConfig.theme ? { background: transparent ? 'rgba(0,0,0,0)' : appConfig.theme } : '', _bgStyle]">
    <slot name="default"></slot>
  </view>
  <!-- #endif -->
  <!-- #ifndef APP-NVUE -->
  <view class="flex relative app flex-1" :style="[appConfig.theme ? { background: transparent ? '' : appConfig.theme } : '', _props.bgImg ? { backgroundImage: `url(${_props.bgImg})` } : '', _bgStyle]">
    <slot name="default"></slot>
  </view>
  <!-- #endif -->
</template>
<script lang="ts" setup>
  ...
  const { dark, isNvue, customCSSStyle, customClass, parentClass, transparent, _props, proxy, blur, round, margin, padding, theme } = useTheme(
    computed(() => props),
    tmcfg,
  )
  //计算主题
