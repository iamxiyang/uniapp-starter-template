<template>
  <button
    ...
    :class="['button alipay flex flex-row flex-row-center-center', isclickOn && !_disabled && !_load ? props.hoverClass + ' bhover' : '', !_disabled && !_load ? 'webpc' : '', _load || _disabled ? 'opacity-8' : '', !isDisabledRoundAndriod ? `round-${btnSizeObj.round}` : '', customClass]"
    :style="[
      {
  ...
  >
    <slot>
      <tm-icon :lineHeight="btnSizeObj.fontSize * 0.9" v-if="_icon && !_load" :userInteractionEnabled="false" :color="_fontColor" :_class="_label ? 'pr-10' : ''" :unit="props.unit" :fontSize="btnSizeObj.fontSize * 0.9" :name="_icon"></tm-icon>
      <!-- #ifdef APP-NVUE -->
...
<script lang="ts" setup>
  ...
  import type { computed, PropType, ref, getCurrentInstance, provide } from 'vue'
